...
const notification = function(data,status){
  const self = this;
  return db.one('INSERT into account_notifications (username,message) VALUES($1,$2) returning id,message,unread',[data.votee,`You were commended on ${status.skillName.skill}! You gained ${status.xp_value} XP`])
          .then(function(notif){
