...
SELECT account.username,
  ...
  is_admin,
  level,
  array_agg(json_build_object('id',ask.id,'skill',ask.skill,'commends',ask.commends)) AS skills
FROM account
LEFT OUTER JOIN Account_skills ask on account.username = ask.username
WHERE account.username= $1 GROUP BY account.username, xp, level, is_admin ;
