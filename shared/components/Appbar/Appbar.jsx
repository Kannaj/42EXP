...
const header = (location, openMemberModal) => {
  if (location === '/projects') {
    ...
    const messageHeaderRegex = location.match('/projects/(\\d+)/((?:[a-zA-Z0-9-_]|%20)+)/messages');
    const name = messageHeaderRegex[2];
    const id = messageHeaderRegex[1];
    return (
      <h3 className="page_title">
        <Link to={`/projects/${id}/${name}`}>{slugify('deslugify', name)} </Link>
        <button className="ion-person-stalker" onClick={() => openMemberModal(name)} />
      </h3>
    );
  ...
  authButtons() {
    if (process.env.NODE_ENV === 'production') {
      return (
        <div className="auth">
          <a href="/auth/github">
            <button className="login_github"> Register With Github</button>
          </a>
        </div>
      );
    }
